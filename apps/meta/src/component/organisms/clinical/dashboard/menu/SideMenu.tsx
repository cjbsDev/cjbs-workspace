...
import { searchState } from 'src/recoil/searchState';
...
const SideMenu = () => {
  ...
  let [checked, setChecked] = useState<CheckType[]>([]);
      ...
      if (type === 'all') {
        ...
        const _updateChecked = updateChecked.map((item) => {
          if (item.code === parentCode) {
            return { ...item, checked: isGroupAllCheck };
          } else {
            return item;
          }
        });
  ...
  const deleteSelectedFilter = async (label: string) => {
    console.log(' label> ', label);

    const filters = await localStorage.getItem(LOCAL_STORAGE_FILTER_KEY);
    if (filters) {
      const filterArray: [] = JSON.parse(filters);
      const deletedFilter = filterArray.filter((item) => item !== label);
      ...
      const deleteChecked = checked.map((item) => {
                          ...
                          <FormControlLabel
                            ...
                            control={
                              <Checkbox
                                ...
                                value={
                                  items.label === 'All'
                                    ? `${checkListIndex}_all_${p_items.field}_${p_items.field}_${items.code}_${items.label}`
                                    : `${checkListIndex}_child_${p_items.field}_${p_items.field}_${items.code}_${items.label}`
                                }
                              />
                            }
                          />
