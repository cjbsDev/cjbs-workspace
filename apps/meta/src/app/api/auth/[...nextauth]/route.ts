...
const refreshAccessToken = mem((
  ...
  const authOptions = (req: NextApiRequest): NextAuthOptions => ({
    providers: [
        CredentialsProvider({
          name: 'Credentials',
          credentials: {
      ...
      callbacks: {
        async jwt ({ token, user, account }) {
          ...
          if(url.indexOf('/api/auth/session?update') > -1){ //업데이트로 실행되면 토큰부터 바로 갱신한다.
            isAutoRefresh = true
          }
          ...
          if (diffMinute <= TOKEN_EALRY_EXPIRE_MINUTE || isAutoRefresh) {
