    ...
    try {
      if (file) {
        ...
        const response = await POST_MULTIPART(
          `/orsh/bs/intn/mtp/${serviceType}/sample`,
          formData
        ); // API 요청
        ...
        >
          ...
          {serviceType === 'fs' ? (
            <SampleTempleteExcelDownloadBtn
              exportUrl={`/file/bs/url?alias=mtpfssamplet`}
              iconName="xls3"
            />
          ) : ('')}
          ...
          {serviceType === 'ngs' ? (
            <SampleTempleteExcelDownloadBtn
              exportUrl={`/file/bs/url?alias=mtpngssamplet`}
              iconName="xls3"
            />
          ) : ('')}
          ...
          {serviceType === 'so' ? (
            <SampleTempleteExcelDownloadBtn
              exportUrl={`/file/bs/url?alias=mtpsosamplet`}
              iconName="xls3"
            />
          ) : ('')}
