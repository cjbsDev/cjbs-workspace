...
const PreviewModal = (props: any) => {
  ...
  const handleAlertOpen = (type: string) => {
    ...
    if (type === "email") {
      setConfirmStr(
        "입력하신 메일 주소로 견적서(이미지) 파일을 첨부하여 발송합니다.\n",
      );
      setConfirmSubStr("메일로 견적서를 발송 하시겠습니까?\n");
    } else if (type === "download") {
      setConfirmStr(
        "견적서(이미지)를 다운로드하면 해당 견적서는 발송 처리 상태로 변경됩니다.",
      );
  ...
  const onSubmit = async (sendType: string) => {
    ...
    try {
      if (sendType === "email" || sendType === "send") {
        // 발송, 상태변경 버튼 클릭시
        ...
        if (response.data.success) {
          ...
          response.data.code === "TRADING_STATEMENT_NOT_EXIST" ||
          response.data.code === "TRADING_STATEMENT_PDF_NOT_EXIST"
        ) {
        ...
        >
          ...
          >
            {resendType === "Y" && (
              <ReactToPrint
                trigger={() => (
                  <OutlinedButton
                    buttonName="인쇄"
                    startIcon={<MyIcon icon="printer" size={20} />}
                  />
                )}
                content={() => ref.current}
              />
            ...
            <Stack spacing={1} alignItems="flex-start" justifyContent="center">
              ...
              <Typography variant="subtitle2">견적서 다운로드</Typography>
              <ContainedButton
                buttonName="견적서 다운로드"
                color={"info"}
