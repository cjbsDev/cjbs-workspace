...
const CustPayInfo = () => {
  ...
  // [선결제 정산내역] 모달
  const [showPrePayListModal, setShowPrePayListModal] =
    useState<boolean>(false);
  const [year, setYear] = useState<string>("2024");

  const { data } = useSWR(
    `/agnc/${anlsItstUkey}/pymt/detail?year=${year}`,
    fetcher,
    {
      suspense: true,
    },
        ...
        <Table>
          <TableBody>
            <TableRow>
              ...
              <TD sx={{ width: "35%" }}>
                <Grid container justifyContent="space-between">
                  ...
                  <Grid item>{/*<AgncDetailInfo agncUkey={agncUkey} />*/}</Grid>
                </Grid>
              </TD>
            ...
            <TableRow>
              ...
              <TD sx={{ width: "35%" }}>{pymtStatusVal}</TD>
      ...
      <TableContainer sx={{ mb: 5 }}>
        <Table>
          <TableBody>
            ...
            <TableRow>
              <TH sx={{ width: "9%" }} align="center">
                분석비용
              </TH>
              {pymtStatusList.map((item: any, index: any) => {
                const { anlsPrice, prcsPymtPrice } = item;
                return (
                  <TD key={index} sx={{ width: "7%" }} align="center">
                    {anlsPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                  </TD>
                );
              })}
            </TableRow>
