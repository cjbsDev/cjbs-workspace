...
const DataTable = () => {
  ...
  const { data } = useSWR(
    JSON.stringify(resultObject) === "{}"
      ? `/inst/list/unRgst?page=${page}&size=${size}`
      : `/inst/list/unRgst${result}&page=${page}&size=${size}`,
    fetcher,
    {
      suspense: true,
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        name: "선택",
        cell: (row) => {
          // const { instUniqueCodeMc, instNm } = row;
          return (
            <OutlinedButton
              size="small"
              buttonName="선택"
              onClick={() => {
                const data = {
                  ...row,
                  // instUniqueCodeMc: instUniqueCodeMc,
                  // instNm: instNm,
                };
                const event = new CustomEvent("myInstData", {
                  detail: data,
                });
